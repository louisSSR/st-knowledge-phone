...
export class SearchService {
  ...
  async search(request: SearchRequest): Promise<SearchResponse> {
    const outcomes = await Promise.allSettled(this.providers.map(async provider => {
      if (!(await provider.isAvailable())) throw new Error(`${provider.name} 当前不可用`);
    ...
    const notice = [...replies.map(reply => reply.notice).filter(Boolean), ...outcomes.flatMap((outcome, index) =>
      outcome.status === 'rejected' ? [`${this.providers[index].name}：${outcome.reason instanceof Error ? outcome.reason.message : '检索失败'}`] : [])].join('；');
