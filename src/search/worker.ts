...
async function loadCorpora(): Promise<SearchCorpus[]> {
  ...
  try {
    ...
    const [manifests, entries, indexes] = await Promise.all([
      read<PackManifest[]>(transaction.objectStore(STORE_NAMES.manifests).getAll()),
      read<StoredEntry[]>(transaction.objectStore(STORE_NAMES.entries).getAll()),
      read<StoredIndex[]>(transaction.objectStore(STORE_NAMES.indexes).getAll()),
    ]);
...
const scope = globalThis as unknown as {
  onmessage: ((event: MessageEvent<{ kind: string; id: number; request: SearchRequest }>) => void) | null;
  postMessage: (message: unknown) => void;
};
scope.onmessage = event => {
  const { kind, id, request } = event.data;
  if (kind !== 'search' || !Number.isSafeInteger(id)) return;
