...
interface HostContext {
  chat: unknown[];
  characters: Array<{ avatar?: unknown; name?: unknown }>;
  characterId?: unknown;
  groupId?: unknown;
...
interface HostApi { getContext(): HostContext; }
...
function identity(context: HostContext): Omit<HostSnapshot, 'messages'> {
  const chatId = identifier(context.getCurrentChatId());
  if (!chatId) return { chatKey: LOBBY_KEY, label: 'SillyTavern · 未选择聊天' };
  ...
  if (!avatar) return { chatKey: LOBBY_KEY, label: 'SillyTavern · 无可用聊天身份' };
...
function textAt(context: HostContext, floor: number): string | null {
  const message = context.chat[floor];
  if (typeof message !== 'object' || message === null || !('mes' in message)) return null;
  const text = message.mes;
  return typeof text === 'string' ? text.slice(0, MAX_MESSAGE_CHARS) : null;
}

function recent(context: HostContext): { snapshot: HostSnapshot; floors: Map<number, string> } {
  ...
  if (who.chatKey !== LOBBY_KEY) {
    ...
    for (let floor = start; floor < context.chat.length; floor++) {
      ...
      if (text !== null) floors.set(floor, text);
    }
  }
...
function hostApi(): HostApi {
  const api = (globalThis as unknown as { SillyTavern?: HostApi }).SillyTavern;
...
export function createSillyTavernAdapter(): HostAdapter {
  const api = hostApi();
  ...
  return {
    label: 'SillyTavern',
    snapshot() {
      if (disposed) throw new Error('SillyTavern Adapter 已释放。');
    ...
    subscribe(callback) {
      ...
      const reconcile = (kind: 'chat' | 'reconcile', nextContext = api.getContext()) => {
        if (!active) return;
      ...
      const message: Listener = (floorValue, generationType) => {
        if (!active) return;
        ...
        if (who.chatKey === LOBBY_KEY || typeof floorValue !== 'number' ||
          !Number.isInteger(floorValue) || floorValue < 0 || floorValue >= nextContext.chat.length) return;
        const text = textAt(nextContext, floorValue);
        if (text === null || floors.get(floorValue) === text) return;
        ...
        while (floors.size > MAX_MESSAGES) floors.delete(floors.keys().next().value!);
        chatLength = nextContext.chat.length;
      ...
      const listen = (name: string, listener: Listener) => {
        ...
        if (typeof type !== 'string' || listeners.some(([event]) => event === type)) return;
      ...
      for (const name of ['MESSAGE_SENT', 'MESSAGE_RECEIVED']) listen(name, message);
      ...
      const unsubscribe = () => {
        if (!active) return;
        active = false;
        for (const [type, listener] of listeners) source.removeListener(type, listener);
        listeners.length = 0;
        floors.clear();
        lastSnapshot = '';
