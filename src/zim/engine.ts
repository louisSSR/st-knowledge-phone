...
interface PendingRequest { action: string; worker: Worker; reject: (error: Error) => void }
...
export class ZimEngine {
  ...
  private uuid = '';
  private opening: Promise<ZimMetadata> | null = null;
  ...
  recoverable(): boolean { return this.file !== null; }
  ...
  async ensureOpen(): Promise<void> {
    ...
    if (!this.file) throw new Error('请先选择一个 ZIM 资料库。');
    await (this.opening ?? this.beginOpening());
  }

  private beginOpening(): Promise<ZimMetadata> {
    if (!this.file) return Promise.reject(new Error('请先选择一个 ZIM 资料库。'));
    const task = this.initialize(this.file, this.uuid, this.generation);
    this.opening = task;
    const clear = () => { if (this.opening === task) this.opening = null; };
    void task.then(clear, clear);
    return task;
  }

  private async initialize(file: File, uuid: string, generation: number): Promise<ZimMetadata> {
    ...
    try {
      ...
      if (generation !== this.generation || this.worker !== worker) throw new Error('资料库打开操作已取消。');
  ...
  async read(path: string): Promise<ZimPage> {
    ...
    if (generation !== this.generation) throw new Error('资料库读取操作已取消。');
  ...
  async diagnostics(): Promise<ZimDiagnostics> {
    const generation = this.generation;
    await this.ensureOpen();
    if (generation !== this.generation) throw new Error('资料库状态读取已取消。');
  ...
  close(error = new Error('资料库已关闭。')): void {
    ...
    this.uuid = '';
    this.opening = null;
    if (this.worker) this.interruptWorker(this.worker, error);
  }
  ...
  private interruptWorker(worker: Worker, error: Error): void {
    if (this.worker !== worker) return;
    this.opened = false;
    this.worker = null;
    worker.terminate();
    for (const item of [...this.pending]) if (item.worker === worker) item.reject(error);
  }
  ...
  private async find(action: string, query: string, options: ZimSearchOptions): Promise<ZimHit[]> {
    ...
    await this.ensureOpen();
    if (generation !== this.generation || searchGeneration !== this.searchGeneration) throw new Error('ZIM 搜索已取消。');
