import type { ContextState } from '../core/types.js';
...
export function extractContext(messages: string[], previous = emptyContext()): ContextState {
  const result: ContextState = { ...previous, location: [...previous.location], recentKeywords: [...previous.recentKeywords] };
  for (const message of messages.slice(-8)) {
    ...
    for (const match of dates) {
      ...
      if (validDate(value)) result.worldDate = value;
    }
    const locations = [...text.matchAll(/(?:地点|位置)\s*[:：]\s*([^\n\r<>\[\]【】]{1,80})/g)];
    if (locations.length) result.location = locations.at(-1)![1].split(/[,，、/｜|]/).map(s => s.trim()).filter(Boolean).slice(0, 4);
