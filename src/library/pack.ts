import type { EntryType, KnowledgeEntry, KnowledgePack, PackManifest, SourceInfo } from '../core/types.js';

export const MAX_PACK_BYTES = 10 * 1024 * 1024;
export const MAX_PACK_ENTRIES = 5000;
export const ENTRY_TYPES: readonly EntryType[] = ['game_rule', 'historical_event', 'sports_match', 'store', 'brand', 'product', 'person', 'place', 'article'];
const DATE_KEYS = ['occurredAt', 'publishedAt', 'knownFrom', 'validFrom', 'validUntil'] as const;
const FORBIDDEN_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

function record(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error(`${label} 必须是对象`);
  const prototype = Object.getPrototypeOf(value);
  if (prototype !== Object.prototype && prototype !== null) throw new Error(`${label} 必须是普通 JSON 对象`);
  if (Object.keys(value).some(key => FORBIDDEN_KEYS.has(key))) throw new Error(`${label} 含保留字段`);
  return value as Record<string, unknown>;
}

function fields(value: Record<string, unknown>, allowed: readonly string[], label: string): void {
  if (Object.keys(value).some(key => !allowed.includes(key))) throw new Error(`${label} 含不支持字段`);
}

function text(value: unknown, label: string, max = 500, empty = false): string {
...
function identifier(value: unknown, label: string): string {
  ...
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(result) || FORBIDDEN_KEYS.has(result)) throw new Error(`${label} 格式无效`);
  return result;
}
...
function date(value: unknown, label: string): string {
  ...
  if (!isDate(result)) throw new Error(`${label} 必须是有效 YYYY-MM-DD 日期`);
  return result;
}

function strings(value: unknown, label: string, max = 40): string[] {
  if (!Array.isArray(value) || value.length > max) throw new Error(`${label} 必须是文本数组（最多 ${max} 项）`);
  return value.map(item => text(item, label, 200));
}

function sourceInfo(input: unknown): SourceInfo {
  const value = record(input, 'source');
  fields(value, ['name', 'url', 'updatedAt', 'license', 'kind'], 'source');
  if (value.kind !== 'offline') throw new Error('仅支持 offline 来源');
  let url: string | undefined;
  if (value.url !== undefined) {
    url = text(value.url, 'source.url', 2048);
    if (!/^https?:\/\//i.test(url)) throw new Error('来源网址仅允许 HTTP(S)');
    try { new URL(url); } catch { throw new Error('来源网址格式无效'); }
  }
...
function entry(input: unknown): KnowledgeEntry {
  ...
  if (!ENTRY_TYPES.includes(value.type as EntryType)) throw new Error('不支持的条目类型');
  const inputDates = record(value.dates, 'dates');
  fields(inputDates, DATE_KEYS, 'dates');
  const dates: KnowledgeEntry['dates'] = {};
  for (const key of DATE_KEYS) if (inputDates[key] !== undefined) dates[key] = date(inputDates[key], `dates.${key}`);
  if (dates.validFrom && dates.validUntil && dates.validFrom > dates.validUntil) throw new Error('有效期起点不能晚于终点');
  const inputMetadata = record(value.metadata, 'metadata');
  if (Object.keys(inputMetadata).length > 40) throw new Error('metadata 字段过多');
  const metadata: KnowledgeEntry['metadata'] = {};
  for (const [key, item] of Object.entries(inputMetadata)) {
    ...
    else if (Array.isArray(item)) metadata[key] = strings(item, 'metadata');
    else metadata[key] = text(item, 'metadata', 2000, true);
  }
  return { id: identifier(value.id, 'entry.id'), type: value.type as EntryType, title: text(value.title, 'title'),
...
function manifest(input: unknown): PackManifest {
  ...
  if (value.schemaVersion !== 1) throw new Error('不支持的 manifest schemaVersion');
...
export function buildDocument(item: KnowledgeEntry, content: string): string {
  return normalizeText([item.title, ...item.aliases, item.summary, ...item.tags, ...item.location, content].join('\n'));
}

export function parsePack(input: unknown): KnowledgePack {
  const value = record(input, '知识包');
  let bytes: number;
  try { bytes = new TextEncoder().encode(JSON.stringify(input)).byteLength; }
  catch { throw new Error('知识包必须可序列化为 JSON'); }
  if (bytes > MAX_PACK_BYTES) throw new Error('知识包超过 10 MB 限制');
  fields(value, ['schemaVersion', 'manifest', 'entries', 'index', 'content'], '知识包');
  if (value.schemaVersion !== 1) throw new Error('不支持的知识包 schemaVersion；请先显式迁移');
  const info = manifest(value.manifest);
  if (!Array.isArray(value.entries) || value.entries.length !== info.entryCount) throw new Error('条目数量与 manifest 不一致');
  const entries = value.entries.map(entry);
  const index = record(value.index, 'index');
  fields(index, ['schemaVersion', 'documents'], 'index');
  if (index.schemaVersion !== 1) throw new Error('不支持的索引 schemaVersion');
  ...
  for (const item of entries) {
    if (Object.hasOwn(documents, item.id) || Object.hasOwn(content, item.contentRef)) throw new Error('条目 ID 或正文引用重复');
    const body = text(inputContent[item.contentRef], '正文', 1_000_000);
    const document = text(inputDocuments[item.id], '索引正文', 1_050_000);
    if (document !== buildDocument(item, body)) throw new Error(`条目 ${item.id} 的预建索引与内容不一致`);
