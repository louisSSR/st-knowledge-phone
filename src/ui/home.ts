...
export function searchForm(ctx: ViewContext, home = false): HTMLFormElement {
  ...
  form.addEventListener('submit', event => {
    ...
    if (query) ctx.search(query, home || ctx.state.settings.sourceMode === 'online' ? [] : ctx.state.types);
    else input.focus();
  });
  return form;
}

export function sourceModeSwitch(ctx: ViewContext): HTMLElement {
  const row = el('div', 'filters');
  row.setAttribute('role', 'group');
  row.setAttribute('aria-label', '搜索来源');
  for (const [mode, label] of [['online', '联网科普'], ['offline', '离线资料']] as const) {
...
export function homeView(ctx: ViewContext): HTMLElement {
  ...
  if (!online) page.append(worldCard(ctx));
