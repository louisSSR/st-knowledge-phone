import type { PackManifest, PhoneState } from '../core/types.js';
...
function packCard(ctx: ViewContext, pack: PackManifest): HTMLElement {
  ...
  if (legacy !== '自定义资料') card.append(el('p', '', '保留既有文件，已从搜索与推荐中撤出；可手动移除。'));
...
function formatBytes(size: number): string {
  if (!Number.isFinite(size) || size <= 0) return '文件大小未提供';
  const units = ['B', 'KiB', 'MiB', 'GiB'];
  const index = Math.min(Math.floor(Math.log(size) / Math.log(1024)), units.length - 1);
  return `${(size / 1024 ** index).toFixed(index ? 1 : 0)} ${units[index]}`;
}

function archiveCard(ctx: ViewContext, archive: PhoneState['archives'][number]): HTMLElement {
  const card = el('article', 'pack');
  ...
  if (!archive.connected) card.append(el('p', '', '重新选择对应的 .zim 文件即可连接。'));
...
function archivePanel(ctx: ViewContext): HTMLElement {
  ...
  input.addEventListener('change', () => {
    ...
    if (file) ctx.run(ctx.controller.attachArchive(file));
    input.value = '';
  });
  label.append(input);
  ...
  for (const [title, url] of [
    ...
    const link = el('a', '', title!); link.href = url!; link.target = '_blank'; link.rel = 'noopener noreferrer'; sources.append(link);
  }
...
export function libraryView(ctx: ViewContext): HTMLElement {
  ...
  online.append(el('span', 'pill', '联网科普来源'), el('h3', '', '中文维基百科'),
    el('p', '', '输入词语即可搜索并阅读来源原文，无需先下载资料库。只发送你主动查询的词，不发送聊天内容。'));
  const lookup = button(ctx.state.settings.sourceMode === 'online' ? '前往百科搜索' : '切换到联网科普', () => ctx.run((async () => {
    if (ctx.state.settings.sourceMode !== 'online') await ctx.controller.setSourceMode('online');
  ...
  for (const result of ctx.state.cachedPages) {
    ...
    read.append(el('span', 'pill', '本机已读缓存'), el('h3', '', result.entry.title),
      el('p', '', `${result.entry.source.name} · 获取于 ${dateLabel}`), el('small', '', '打开缓存正文 · 不联网刷新'));
    read.disabled = ctx.state.busy;
    cached.append(read);
  }
  if (!ctx.state.cachedPages.length) cached.append(emptyState('读过的原文会留在这里', '打开联网百科正文后，可从这里再次阅读已保存的页面。断网时也能打开已有缓存。'));
  page.append(cached);
  const offline = el('details', 'import-panel');
  offline.open = ctx.state.settings.sourceMode === 'offline' || ctx.state.canRetryArchive;
  offline.append(el('summary', '', `可选 · 离线知识库（${ctx.state.archives.length} 个）`),
    el('p', 'settings-caption', '需要断网检索大量资料时，可连接已下载的 ZIM 文件。联网科普和已读缓存不需要安装它。'));
  ...
  if (!ctx.state.archives.length) archives.append(emptyState('尚未连接知识库', '从上方选择现成的 .zim 文件，即可开始查找原文。'));
