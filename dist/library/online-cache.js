...
export function sourceText(html) {
    const plain = html.replace(/<(script|style)\b[^>]*>[\s\S]*?<\/\1\s*>/gi, '')
        .replace(/<[^>]*>/g, ' ').replace(/&#(x[0-9a-f]+|\d+);/gi, (_match, digits) => {
        const code = digits[0].toLowerCase() === 'x' ? parseInt(digits.slice(1), 16) : Number(digits);
        return code > 0 && code <= 0x10ffff ? String.fromCodePoint(code) : '';
    });
    return plain.replace(/&(nbsp|amp|lt|gt|quot|apos);/g, (_match, entity) => ({ nbsp: ' ', amp: '&', lt: '<', gt: '>', quot: '"', apos: "'" }[entity] ?? ''))
        .replace(/\s+/g, ' ').trim();
}
export function cachedResult(page) {
...
export class OnlineCache {
    ...
    async list() {
        ...
        const saved = await this.storage.getValue(ONLINE_CACHE_KEY);
        if (saved?.schemaVersion !== 1 || !Array.isArray(saved.pages))
            return [];
    ...
    put(page) {
        const operation = this.writes.catch(() => undefined).then(async () => {
            ...
            if (encoder.encode(JSON.stringify(page)).length > ONLINE_CACHE_MAX_BYTES)
                return false;
            const saved = await this.storage.getValue(ONLINE_CACHE_KEY);
            ...
            while (encoder.encode(JSON.stringify({ schemaVersion: 1, pages })).length > ONLINE_CACHE_MAX_BYTES)
                pages.pop();
            if (!pages.length)
                return false;
            await this.storage.setValue(ONLINE_CACHE_KEY, { schemaVersion: 1, pages });
            return true;
        });
