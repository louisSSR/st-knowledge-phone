import { OnlineCache, cachedResult, normalizedTerm, sourceText } from '../library/online-cache.js';
...
export function normalizeOnlinePath(input) {
    const raw = input.trim();
    if (!raw || raw.length > 1200)
        throw new Error('词条地址无效');
    let title = raw;
    if (/^(?:https?:\/\/|\/\/|\/wiki\/|\/w\/index\.php)/i.test(raw)) {
        const url = new URL(raw, 'https://zh.wikipedia.org');
        if (url.origin !== 'https://zh.wikipedia.org' || url.username || url.password)
            throw new Error('此链接不属于中文维基百科');
        if (url.pathname.startsWith('/wiki/'))
            title = decodeURIComponent(url.pathname.slice(6));
        else if (url.pathname === '/w/index.php')
            title = url.searchParams.get('title') ?? '';
        else
            throw new Error('此链接不是词条页面');
    }
    else {
        if (/^[a-z][a-z\d+.-]*:\/\//i.test(raw) || /^(?:javascript|data|file|blob):/i.test(raw))
            throw new Error('词条地址无效');
        title = raw.replace(/^\.\//, '').split('#')[0];
        try {
            title = decodeURIComponent(title);
        }
        catch { /* Plain text percent signs can be part of a title. */ }
    }
...
function resultFromHit(hit, fetchedAt, score, reason) {
...
function aborted() { return new DOMException('查询已取消', 'AbortError'); }
export class OnlineProvider {
    ...
    async isAvailable() { return !this.disposed; }
    cancelSearch() { this.generation++; for (const controller of this.pending)
        controller.abort(); this.pending.clear(); }
    pause() { this.cancelSearch(); }
    dispose() { this.disposed = true; this.cancelSearch(); }
    async api(parameters) {
        if (this.disposed)
            throw aborted();
        if (this.now() < this.retryAfter)
            throw new Error('来源暂时限流，请稍后重试');
        ...
        try {
            ...
            if (new TextEncoder().encode(text).length > MAX_RESPONSE_BYTES)
                throw new Error('来源页面过大，请在原站阅读');
            const data = JSON.parse(text);
            if (data.error)
                throw new Error(`来源读取失败：${data.error.info ?? data.error.code ?? '未知错误'}`);
            return data;
        }
        catch (error) {
            if (timedOut)
                throw new Error('中文维基百科连接超时（8 秒），请检查网络后重试');
    ...
    async search(request) {
        ...
        if (!query || (request.types?.length && !request.types.includes('article')))
            return { results: [], total: 0, suggestions: [] };
        if (request.context.strictTimeline)
            return { results: [], total: 0, suggestions: [], notice: '联网来源是当前版本，未经历史版本核验；严格剧情模式下不展示。日常科普可使用科普模式。' };
        ...
        try {
            const [searched, exact] = await Promise.allSettled([
                ...
                query.includes('|') ? Promise.resolve({}) : this.api({ action: 'query', titles: query, redirects: '1', converttitles: '1', prop: 'info' }),
            ]);
            if (generation !== this.generation || this.disposed)
                throw aborted();
            if (searched.status === 'rejected')
                throw searched.reason;
            if (!Array.isArray(searched.value.query?.search))
                throw new Error('来源没有返回有效搜索结果');
            return this.searchResponse(searched.value, exact.status === 'fulfilled' ? exact.value : undefined, request, query);
        }
        catch (error) {
            if (generation !== this.generation || this.disposed || (error instanceof DOMException && error.name === 'AbortError'))
                throw aborted();
            let results = [];
            try {
                results = await this.cache.search(query, MAX_RESULTS);
            }
            catch { /* The network failure remains useful when storage is also unavailable. */ }
            if (generation !== this.generation || this.disposed)
                throw aborted();
    ...
    searchResponse(searched, exact, request, query) {
        ...
        const results = hits.map((page, index) => resultFromHit(page, timestamp, 500 - index, normalizedTerm(page.title) === normalizedTerm(query) ? '标题精确匹配 · 来源检索' : '来源全文检索'));
        if (hit) {
            const existing = results.find(result => result.entry.metadata.pageId === hit.pageid || result.entry.title === hit.title);
            if (existing) {
                existing.score = 1000;
                existing.reason = '标题或来源重定向精确匹配';
                existing.entry.metadata.revision = hit.lastrevid ?? 0;
            }
            else
                results.unshift(resultFromHit(hit, timestamp, 1000, '标题或来源重定向精确匹配'));
            const result = results.find(row => row.entry.title === hit.title);
            if (result && result.entry.title !== query)
                result.entry.aliases = [query];
        }
    ...
    async read(input) {
        ...
        if (typeof input !== 'string' && input.entry.source.kind === 'cache') {
            ...
            if (generation !== this.generation || this.disposed)
                throw aborted();
            if (cached)
                return { result: cachedResult(cached), content: cached.content, cached: true, notice: '正在阅读已读原文缓存。' };
        }
        try {
            ...
            if (!page || typeof page.text !== 'string' || !page.text.trim() || !page.title || !Number.isSafeInteger(page.revid))
            ...
            try {
                ...
                if (!saved)
                    notice = '原文已读取；页面超过本地缓存预算，本次未保存。';
            }
            catch {
                notice = '原文已读取，但浏览器未能保存离线缓存。';
            }
            if (generation !== this.generation || this.disposed)
                throw aborted();
