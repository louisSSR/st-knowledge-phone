...
function topbar(controller, run, close) {
...
export function mountPhone(shadow, controller, onClose) {
    ...
    let activeReaderPath;
    let activeReaderHash;
    const run = (task) => {
        localError = '';
        task.catch(error => {
            if (disposed)
    ...
    const search = (query, types, nextOffset = 0) => {
        offset = nextOffset;
        resetScroll = true;
        lastQuery = query;
        run(controller.search(query, types, nextOffset));
    };
    const navigate = (page, nextOffset = 0) => {
        offset = nextOffset;
        resetScroll = true;
    ...
    const render = (state) => {
        ...
        if (state.reader && !oldView.startsWith('reader:'))
            readerOrigin = { focus, scroll: content.scrollTop, page: state.page };
        ...
        const ctx = { state, controller, offset, run, search, navigate };
        ...
        if (preserveReader)
            ...
            if (state.reader) {
                ...
                content.append(views[state.page](ctx));
        }
        renderNavigation(nav, state, controller, run, () => { offset = 0; });
        if (!changedView && state.chatKey === renderedChat)
            restoreDraft(content, draft, state.query !== renderedQuery);
        content.scrollTop = scroll;
        if (returning && restoreFocus(shadow, readerOrigin.focus)) {
            readerOrigin = null;
        }
        else if (changedView && oldView) {
            const target = state.reader ? content.querySelector('[data-reader-title]') : content.querySelector('input, h2');
            if (target) {
            ...
            restoreFocus(shadow, focus);
        oldView = viewKey;
        renderedQuery = state.query;
        renderedChat = state.chatKey;
    };
    const keydown = (event) => handleKeyboard(event, shadow, controller, onClose);
...
function renderNavigation(nav, state, controller, run, resetOffset) {
    nav.replaceChildren();
...
function handleKeyboard(event, shadow, controller, onClose) {
    event.stopPropagation();
    if (event.key === 'Escape') {
        event.preventDefault();
        if (controller.getState().reader)
    ...
    if (event.key !== 'Tab')
        return;
    const nodes = [...shadow.querySelectorAll(focusable)].filter(node => node.getClientRects().length > 0);
    const first = nodes[0];
