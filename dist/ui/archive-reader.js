...
function archiveLocation(raw, path) {
...
function safeDeclarations(style) {
    ...
    for (let index = 0; index < style.length; index++) {
        ...
        if (/url\s*\(|image-set\s*\(|image\s*\(|paint\s*\(|expression\s*\(|(?:https?|data|blob|javascript):|\/\//i.test(normalized)
            || /behavior|binding|animation|transition/i.test(property) || property.startsWith('--'))
            continue;
        ...
        function rules(input, depth = 0) {
            ...
            return [...input].map(rule => {
                ...
                if (rule.type === CSSRule.MEDIA_RULE || rule.type === CSSRule.SUPPORTS_RULE) {
                    ...
                    if (/url\s*\(|https?:|[{}]/i.test(condition))
                        return '';
...
export function createArchiveReader(ctx) {
    ...
    function updateStatus() {
        ...
        if (mathCount)
            notes.push(`${mathCount} 个公式使用原文 MathML 呈现，图像回退不重复加载`);
    ...
    for (const wrapper of template.content.querySelectorAll('.mwe-math-element')) {
        if (wrapper.querySelector('math'))
            wrapper.querySelectorAll('img.mwe-math-fallback-image-inline,img.mwe-math-fallback-image-display').forEach(image => image.remove());
    }
    let imageJobs = 0;
    for (const node of [...template.content.querySelectorAll('*')]) {
        if (!template.content.contains(node))
            continue;
    ...
    async function loaded() {
        ...
        function resize() {
            ...
            frameId = requestAnimationFrame(() => {
                if (!disposed && iframe.contentDocument === doc)
                    iframe.style.height = `${Math.min(12000, Math.max(320, doc.body.scrollHeight + 6))}px`;
            });
        }
        ...
        function initialHash() {
            ...
            hashFrameId = requestAnimationFrame(() => {
                if (!disposed && iframe.contentDocument === doc)
                    scrollHash(reader.hash);
            });
        }
        function click(event) {
            const button = event.button;
            if (typeof button === 'number' && button !== 0 && button !== 1)
                return;
        ...
        async function resource(path) {
            ...
            if (!promise) {
                if (++resources > MAX_RESOURCES)
                    throw new Error('本页资源数量超过上限');
                promise = ctx.controller.readArchiveResource(packId, path).then(value => {
                    if (disposed)
                        throw new Error('阅读已结束');
                    totalBytes += value.data.byteLength;
                    if (value.data.byteLength > MAX_IMAGE_BYTES || totalBytes > MAX_TOTAL_BYTES)
                        throw new Error('本页资源大小超过上限');
                    return value;
                });
        ...
        async function worker() {
            while (!disposed) {
                ...
                try {
                    ...
                    if (job.kind === 'css') {
                        if (mime !== 'text/css' || value.data.byteLength > MAX_CSS_BYTES)
                            throw new Error('样式类型或大小不支持');
                        ...
                        if (!/^image\/(?:png|jpeg|gif|webp|avif|bmp|x-icon|vnd\.microsoft\.icon)$/.test(mime))
                            throw new Error('图片格式未启用');
                        ...
                        image.addEventListener('error', () => { if (!disposed) {
                            loadedImages--;
                            failedImages++;
                            updateStatus();
                            resize();
                    ...
                    if (job.kind === 'css')
                        ...
                        if (image) {
                            ...
                            fallback.className = 'kp-image-unavailable';
