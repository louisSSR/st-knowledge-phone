...
以下链接固定到同一提交，可复核相同实现。来源匹配置信度为高；**仅源码验证，尚无真实宿主运行验收**。未对 1.14–1.17 作兼容承诺，也没有把这一源码版本当作目前最新版本。
...
下一验收门：在另行授权的真实 ST 1.18.0 实例中测试切换同名聊天的不同角色、群聊、新建聊天、编辑/续写/swipe/删除、手机关闭后的上下文更新、禁用与重新启用的监听清理，以及账号切换边界。静态和模拟事件测试均不能设置 `driver-accepted`。
